...
1. THE Onboarding_Modal SHALL display exactly 7 sequential steps
...
5. THE Onboarding_Modal SHALL display the current step number and total steps (e.g., "Step 2 of 7")
 
### Requirement 4: User Information Collection
 
**User Story:** As a new user, I want to provide my username and AWSCC ID during onboarding, so that I can create my profile and participate in the community.
 
#### Acceptance Criteria
 
1. THE Onboarding_Modal SHALL display a form at Step 2 with fields for username and AWSCC ID
2. THE Frontend SHALL validate that username contains only alphanumeric characters, hyphens, and underscores
3. THE Frontend SHALL validate that AWSCC ID is not empty
...
2. THE Onboarding_Modal SHALL display a "Mark as Done" checkbox or button for Steps 4, 5, and 6
...
1. THE Onboarding_Modal SHALL display Step 4 (UI/UX Setup) with an accordion component
2. THE Onboarding_Modal SHALL display Step 5 (Frontend Setup) with an accordion component
3. THE Onboarding_Modal SHALL display Step 6 (Deployment Setup) with an accordion component
4. WHEN a User clicks an accordion section header, THE Frontend SHALL expand that section and display its content
5. THE Frontend SHALL allow multiple accordion sections to be open simultaneously within a single step
...
4. THE step_number column SHALL store integer values from 1 to 7
